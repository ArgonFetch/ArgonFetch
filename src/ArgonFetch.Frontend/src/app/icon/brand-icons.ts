...
/**
 * Marks belonging to somebody else.
 * <p>
 * Lucide dropped brand icons in v1 and will not be taking them back - a logo is a trademark
 * rather than a piece of interface vocabulary, so it is not theirs to redraw. The ones we
 * genuinely need are kept here in the shape Lucide uses, so they draw through the same
 * component and inherit its size and colour like any other icon.
 * <p>
 * Unlike a Lucide icon these are solid shapes rather than strokes, so each one turns off the
 * stroke and fills instead. An attribute on the shape wins over the one the component sets on
 * the surrounding svg, so no special case is needed to draw them.
 */
export const GithubMark: IconNode = [
  ['path', {
    fill: 'currentColor',
    stroke: 'none',
